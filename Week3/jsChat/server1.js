...
const Packet = {
	...
	buldList:function(arrOfClients){
...
class Client{
	...
	onData(data){
		...
		//console.log(packets.length + " new data recieved from " + this.socket.localAddress);
	...
	handlePacket(packet){
		...
		switch(parts[0]){
			...
			case"NAME":
				...
				//TODO: accept of reject new name
...
class Server{
	constructor(){
		...
		this.clients = []; // list of currently connected clients
	...
	onClientDisconnect(client){
		...
		this.clients.splice(this.server.clients.indexOf(client), 1);
		//TODO: broadcast a LIST packet to everyone
	}
	onError(errMsg){
